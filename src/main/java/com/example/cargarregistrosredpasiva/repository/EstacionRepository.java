...
public interface EstacionRepository extends JpaRepository<Estacion, Integer> {
    ...
    @Query(value = "insert into main_estacion (identificador, nomenclatura, estado_vertice,  municipio, latitud, longitud, altura_elipsoidal, agencia, geom) values (?,?, ?,?, ?, ?,?, ?, ST_Transform(ST_SetSRID(ST_MakePoint(?, ?), 4326), 9377) returning *", nativeQuery = true)
